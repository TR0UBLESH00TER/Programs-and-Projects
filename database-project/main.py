...
def display():
    ...
    Label(frame, text='Search by Entering Admission Number', font=('Courier', 10)).grid(row=0, column=1, pady=5)

    # Entry widget to get details of specified Admission_No
    search = Entry(frame,width=17)
    search.grid(row=0,column=2)
    Button(frame, text='Search', font=('Courier', 10),padx=15,command=Search).grid(row=0, column=3, pady=5)

    # Button to generate csv file of all data
    Button(frame, text='Get csv', font=('Courier', 10),padx=15,command=export_csv).grid(row=0, column=4)

    # Creating tree's frame
    tree_frame = Frame(frame)
    tree_frame.grid(row=1,column=0,columnspan=5,pady=5)
    ...
    # Creating Tree
    tree = ttk.Treeview(tree_frame, selectmode='browse',height=20)
    tree.pack()

    # Configuring y_scroll
    tree.configure(yscrollcommand=y_scroll.set)
    y_scroll.config(command = tree.yview)
    
    #Defining columns
    tree["columns"] = ('Admission No','Name','Class','Section','Phone Number','DOB','DOA')

    # Formatting Columns
    tree.column("#0",width=0,stretch=NO)
    tree.column("Admission No", minwidth=85,  width=85,  anchor='c')
    tree.column("Name",         minwidth=200, width=200, anchor='w')
    tree.column("Class",        minwidth=60,  width=60,  anchor='c')
    tree.column("Section",      minwidth=60,  width=60,  anchor='c')
    tree.column("Phone Number", minwidth=120, width=120, anchor='c')
    tree.column("DOB",          minwidth=90,  width=90,  anchor='c')
    tree.column("DOA",          minwidth=90,  width=90,  anchor='c')
